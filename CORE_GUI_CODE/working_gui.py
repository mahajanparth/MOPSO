...
def Plan_mission_func():
    ...
    print("gnome-terminal -- sim_vehicle.py -v ArduCopter -L hindan --map &")
    subprocess.check_call("gnome-terminal -- sim_vehicle.py -v ArduCopter -L hindan --map &", shell=True)
...
def Upload_mission_func():
    ...
    for ip, port in uav_dict.values():
        try:
            ...
            not_sucessful.append(ip)
    print("successfully_send to ", successful)
    print("not_successfully_send to ", not_sucessful)
...
home_layout = [
    ...
    [sg.Text('Enter the minimum distance between UAVs', size=(36, 1), justification='right'),
    ...
    [sg.Button('Start Mission', button_color=('white', 'green')),
     sg.Button('ARM ALL', button_color=('white', 'green'))],
